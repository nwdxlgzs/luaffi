...
#include <string.h>
#ifdef _WIN32
typedef unsigned int gid_t;        // 组 ID
typedef unsigned int uid_t;        // 用户 ID
typedef unsigned long nlink_t;     // 硬链接数量
typedef long suseconds_t;          // 时间的微秒部分
typedef int blksize_t;             // 块大小
typedef long long blkcnt_t;        // 块计数
#define WIN_RTLD_DEFAULT NULL
#include <malloc.h>
#include <windows.h>
#include <psapi.h>
#else
...
#ifdef _WIN32
    if (lib->h == WIN_RTLD_DEFAULT) {
        HANDLE process = GetCurrentProcess();
        HMODULE modules[1024];
        DWORD cbNeeded;
        EnumProcessModules(process, modules, sizeof(modules), &cbNeeded);
...
static int cparse_function(lua_State* L, int tok, struct ctype* rtype) {
    ...
    if (cparse_check_tok(L, tok) != TOK_NAME)
        return cparse_expected_error(L, tok, "identifier");
    char* yyname = yyget_text();
    if (yyname && strlen(yyname) < sizeof(name))
        strcpy(name, yyname);
    else
        strcpy(name, "anonymous|longname");
    lua_pushstring(L, yyname);
    ...
    if (cparse_check_tok(L, tok) != ';')
        return cparse_expected_error(L, tok, ";");
    const int func_size = sizeof(struct cfunc) + sizeof(struct ctype*) * narg;
    func = calloc(1, func_size);
    if (!func)
        return luaL_error(L, "no mem");
...
static int load_lib(lua_State* L, const char* path, bool global) {
    ...
    if (path) {
        ...
        if (!h) {
            ...
            return luaL_error(L, "LoadLibrary() failed: %d", (int)err);
        }
#else
